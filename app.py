...
/* 그라데이션 클립이 이모지를 실루엣(■)으로 만드는 것 방지 — 이모지는 원색으로 */
.hero h1 .emoji {
...
if check_btn:
    ...
    st.markdown('<div class="hero"><h1><span class="emoji">🔔</span> 워치리스트 점검</h1>'
...
if view == "🔎 직접 종목 검색":
    if not search_pick_ticker:
        st.markdown("""
        <div class="hero">
          <h1><span class="emoji">🔎</span> 개별 종목 검색</h1>
...
if view == "📊 전체 종목 분석":
    ...
    if loaded_verdicts:
        ...
        st.markdown(f"""
        <div class="hero">
          <h1><span class="emoji">⏳</span> {mk_name} 자동 수집 대기 중</h1>
...
if _rs_now == "auto:kr":   _mkt_badge = '<span class="emoji">🇰🇷</span> 국장 '
elif _rs_now == "auto:us": _mkt_badge = '<span class="emoji">🇺🇸</span> 미장 '
...
st.markdown(f"""
<div class="hero">
  <h1><span class="emoji">📊</span> {_mkt_badge}분석 결과</h1>
